...
class SMPLXSKinningBatch(nn.Module):
    def __init__(self, use_agg=False, human_sample=True, density_loss=False, with_viewdirs=False, use_f2d=True, use_trans=False, 
        ...
        super(SMPLXSKinningBatch, self).__init__()
    ...
    def register_smplx_params(self):
        # If not add the following line, the smplx model construction will corrupt during create mean pose.
        ...
        model_init_params = dict(
            ...
            create_transl=False,
            num_pca_comps=12) 
        smplx_male_model = smplx.create(**model_init_params)
        self.SMPL_MALE = smplx_male_model
        model_init_params['gender'] = 'female'
        smplx_female_model = smplx.create(**model_init_params)
        self.SMPL_FEMALE = smplx_female_model
        model_init_params['gender'] = 'neutral'
        smplx_neutral_model = smplx.create(**model_init_params)
        self.SMPL_NEU = smplx_neutral_model
        self.SMPL_NEUTRAL = self.SMPL_NEU
        # set default tensor type back to cuda tensor
        torch.set_default_tensor_type('torch.cuda.FloatTensor')
        self.register_buffer(
            'faces',
            torch.tensor(self.SMPL_NEUTRAL.faces.astype(np.int32)).long()
        )
    ...
    def coarse_deform_target2c(self, params, vertices,  query_pts):
        ...
        A, R, Th, joints = get_smplx_transform_params_torch(self.SMPL_NEUTRAL, params)        
        J = torch.hstack((joints, torch.ones([joints.shape[0], 1]).cuda()))
        ...
        # From T To Big Pose        
        A, R, Th, joints = get_smplx_transform_params_torch(self.SMPL_NEUTRAL, params, bigparams=True)
    ...
    def coarse_deform_c2source(self, params, t_vertices, query_pts, weights_correction):
        ...
        smpl_pts = t_vertices
        _, vert_ids, _ = knn_points(query_pts.unsqueeze(0).float(), smpl_pts.unsqueeze(0).float(), K=1)
        nJ = self.SMPL_NEUTRAL.lbs_weights.shape[1]
        bweights = self.SMPL_NEUTRAL.lbs_weights[vert_ids.squeeze(0)].view(-1,nJ).cuda()
        ...
        A, R, Th, joints = get_smplx_transform_params_torch(self.SMPL_NEUTRAL, params)
        self.s_A = A
        A = torch.mm(bweights, self.s_A.reshape(nJ, -1))
        A = torch.reshape(A, (-1, 4, 4))
    ...
    def forward(self, sp_input, tp_input, world_query_pts, viewdir):
        ...
        if self.skinning_field:
            ...
            fused_feature = self.backward_fusion(canonical_pts.clone(), embedding=embedding)
        ...
        if self.human_sample:
            ...
            if self.smooth_loss and self.training:
                ...
                if intv_flag:
                    ...
                    del x, h, point_2d_feature, point_2d_feature_0, weights_correction
                    del feature, canonical_pts, smpl_src_pts, world_src_pts
                    del coarse_canonical_pts, pts_mask, sp_input, tp_input, world_query_pts, viewdir, occ_normal
                    if self.skinning_field:
                        del fused_feature, instance_idx
        else:
            pts_mask = torch.ones_like(smpl_query_pts[:,0]).cuda().int()
